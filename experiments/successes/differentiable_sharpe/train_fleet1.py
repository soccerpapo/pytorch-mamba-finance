...
# 👇 NEW FILENAME: So we don't overwrite the 'Frozen Sniper'
SAVE_PATH = os.path.join(current_dir, "champion_model1.pth")

# Lower fee for training to encourage movement
TRAIN_FEE = 0.001
...
def train_one_candidate(features, run_id):
    ...
    for epoch in range(EPOCHS_PER_RUN):
        ...
        for batch in train_loader:
            ...
            # 1. The Main Objective (Sharpe)
            sharpe_loss = differentiable_sharpe_loss(positions, returns, transaction_cost=TRAIN_FEE)
            
            # 2. The Cure: Activity Bonus
            # "Don't just sit there!" (0.05 is the weight of this 'nagging')
            # "Anti-Lasso" regularization
            activity_bonus = -0.002 * torch.mean(torch.abs(positions))
            
            total_loss = sharpe_loss + activity_bonus
            
            total_loss.backward()
            optimizer.step()
            
    # Validation (Strict Mode: No bonus)
    model.eval()
    val_sharpe = 0
    batches = 0
    with torch.no_grad():
...
def main():
    print(f"🔥 STARTING TRAINING V2 (Activity Bonus)")
